...
==== Basic Settings
Startup settings are specified on this tab.
When enabled the "On database unlock, show entries that" setting will apply a search filter that is displayed in the Search field and applied when the database is opened or unlocked. Disable the setting to prevent a search from being applied when the database is opened or unlocked.

.On database unlock
image::on_database_unlock.png[]
