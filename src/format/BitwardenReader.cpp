    ...
    Entry* readItem(const QJsonObject& item, QString& folderId)
    {
        ...
        if (itemMap.contains("login")) {
            ...
            for (const auto& urlObj : loginMap.value("uris").toList()) {
                auto url = urlObj.toMap().value("uri").toString();
        ...
        if (itemMap.contains("identity")) {
            ...
            auto sensitive = QStringList({"ssn", "passportNumber", "licenseNumber"});
            for (const auto& attr : attrs) {
                auto value = idMap.value(attr).toString();
                if (!value.isEmpty()) {
            ...
            auto username = idMap.value("username").toString();
            if (!username.isEmpty()) {
        ...
        if (itemMap.contains("card")) {
            auto cardMap = itemMap.value("card").toMap();
            auto attrs = QStringList({"cardholderName", "brand", "number", "expMonth", "expYear", "code"});
            auto sensitive = QStringList({"code"});
            for (const auto& attr : attrs) {
